...
async function submitPort(req,res){
  try {
    ...
    if (signature) {
      ...
      console.log(valid);
      let signatureHash = await arweave.crypto
      .hash(Buffer.from(JSON.stringify(dataAndSignature.signature)));
...
/**
 * Check if given hash is of given difficulty
 * @param {string} hash express.js request
 * @return  {boolean} returns true if hash has desired difficulty level else false
 */
